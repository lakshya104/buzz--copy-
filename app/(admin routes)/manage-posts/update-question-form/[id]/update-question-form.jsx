...
import { updateQuestionAndAnswers } from "@/actions/server-utils";
...
const UpdateQuestionForm = ({ text, answers, id, feedId }) => {
  ...
  function onSubmit(values) {
    ...
    setDisable(true);
    router.push(`/manage-posts/manage-question/${feedId}?title=${title}`);
    router.refresh();
    toast({
  ...
  return (
    <div className="mx-auto my-1 p-1 w-full">
      <div className="flex justify-start items-center mb-6">
        <ProgressBarLink
          href={`/manage-posts/manage-question/${feedId}?title=${title}`}
          className="w-[30%] ml-2"
        >
